...
let first = "aa bb cc dd ee"
let second = "aa bb cc dd aa"
let third = "aa bb cc dd aaa"

func checkValidity(passphrase: String) -> Bool {
    let split = passphrase.components(separatedBy: CharacterSet.whitespaces)
    let unique = Set(split)
    return unique.count == split.count
}
checkValidity(passphrase: first)
checkValidity(passphrase: second)
checkValidity(passphrase: third)

puzzleInput.count

let p1result = puzzleInput
    .components(separatedBy: CharacterSet.newlines)
    .filter({checkValidity(passphrase: $0)})
    .count
...
func checkValidityAnagrams(passphrase: String) -> Bool {
...
let p2result = puzzleInput
    .components(separatedBy: CharacterSet.newlines)
    .filter({checkValidityAnagrams(passphrase: $0)})
    .count
