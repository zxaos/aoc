...
class Part1Tests : XCTestCase {
    @objc func testAssertions() {
        let test1 = """
                    ...
                    2 4 6 8
                    """
        XCTAssertEqual(checksum(sheet: test1), 18)
    }
}

func parseSheet(sheet: String) -> [ [Int] ] {
    return sheet.components(separatedBy: CharacterSet.newlines)
        .map { $0.components(separatedBy: CharacterSet.whitespaces).filter {!$0.isEmpty} }
        .map { $0.map { Int($0)! }  }
}

func rowDiff(_ row: [Int] ) -> Int {
    return row.max()! - row.min()!
}

func checksum (sheet: String) -> Int {
    return parseSheet(sheet: sheet)
        .map(rowDiff)
        .reduce(0, +)
}
...
print (checksum(sheet: d2PuzzleInput))
...
class Part2Tests : XCTestCase {
    @objc func testAssertions() {
        let test2 = """
                    ...
                    3 8 6 5
                    """
        XCTAssertEqual(rowMod([5,9,2,8]), 4)
        XCTAssertEqual(rowMod([9,4,7,3]), 3)
        XCTAssertEqual(rowMod([3,8,6,5]), 2)
        XCTAssertEqual(sheetMod(sheet: test2), 9)
        
    }
}

func rowMod (_ row: [Int] ) -> Int {
...
func sheetMod (sheet: String) -> Int {
    return parseSheet(sheet: sheet)
        .map(rowMod)
        .reduce(0, +)
}
...
print(sheetMod(sheet: d2PuzzleInput))
